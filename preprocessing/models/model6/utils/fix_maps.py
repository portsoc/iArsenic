...
def fix_maps():
    ...
    upazilas = gpd.read_file('../../geodata/maps/dist/upa/upa-c005-s020-vw-pr.geojson')
    unions = gpd.read_file('../../geodata/maps/dist/uni/uni-c005-s020-vw-pr.geojson')
    mouzas = gpd.read_file('../../geodata/maps/dist/mou/mou-c005-s020-vw-pr.geojson')
    ...
    dis.to_file('output/geodata/intersected_districts.geojson', driver='GeoJSON')
    upa.to_file('output/geodata/intersected_upazilas.geojson', driver='GeoJSON')
    uni.to_file('output/geodata/intersected_unions.geojson', driver='GeoJSON')
    mou.to_file('output/geodata/intersected_mouzas.geojson', driver='GeoJSON')

    dis = gpd.read_file('output/geodata/intersected_districts.geojson')
    dis_topo = json.loads(
        tp.Topology(dis).to_json()
    )

    with open('output/geodata/intersected_districts.topojson', 'w') as f:
        json.dump(dis_topo, f)

    upa = gpd.read_file('output/geodata/intersected_upazilas.geojson')
    upa_topo = json.loads(
        tp.Topology(upa).to_json()
    )

    with open('output/geodata/intersected_upazilas.topojson', 'w') as f:
        json.dump(upa_topo, f)

    uni = gpd.read_file('output/geodata/intersected_unions.geojson')
    uni_topo = json.loads(
        tp.Topology(uni).to_json()
    )

    with open('output/geodata/intersected_unions.topojson', 'w') as f:
        json.dump(uni_topo, f)

    mou = gpd.read_file('output/geodata/intersected_mouzas.geojson')
    mou_topo = json.loads(
