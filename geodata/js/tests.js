...
function runGeoLocTests() {
  ...
  expectNull('left of corner 2', findRegion(-0.0001, 1));
  expectNull('up of corner 2', findRegion(0, 1.0001));
  expectNull('outside of line between 1 and 2', findRegion(0.49, 0.5));
  expectNull('between shapes', findRegion(1.5, 1));

  expectNonNull('off corner 2, inside', findRegion(0.0001, 1));
  expectNonNull('inside of line between 1 and 2', findRegion(0.51, 0.5));
}
...
function testHLineConsistency(lon1, lon2, lat) {
  ...
  for (let i = 2; i < N; i += 1) {
    if (expectedResult !== findRegion(between(lon1, lon2, i / N), lat)) { return false; }
  }
  return true;
}
...
function testVLineConsistency(lon, lat1, lat2) {
  ...
  for (let i = 2; i < N; i += 1) {
    if (expectedResult !== findRegion(lon, between(lat1, lat2, i / N))) { return false; }
  }
  return true;
}
