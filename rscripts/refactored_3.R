...
if (length(wells_in_area) > 0){
  ...
  if ((input$colour == 'Black' | input$utensil == 'No colour change to slightly blackish')) {
    ...
    if (depth < 90){
      beginning_text =
        if ((depth <= 15) && (input$colour == 'Red') && (as_median_under_90 >= 20)) {
          'in'
        } else {
          'Your tubewell is'
        }
      ...
      paste (beginning_text, pollution_status_under_90, chem_test_status, round.choose (lower_quantile_under_90, 10,1), 'to', round.choose (upper_quantile_under_90, 10,1),'µg/L ')
